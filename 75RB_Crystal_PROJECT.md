...
- **สรุปวิธีเก็บที่ดีที่สุด (GrabTest 2026-08-06)**: `CrystalHoldComplete:FireServer(ก้อน)`
  **ยิงตรงเข้าเลย ไม่ต้องกดค้าง ไม่ต้องรอปุ่มติด** — ทดสอบ 15+ ก้อน เข้าทุกก้อน:
  T1-T5 / 0.5-20.9 kg / ถึง $204,533 / Hold 1-4 วิ → ของเข้าใน **0.2-1.7 วิ**
  ⚠ เงื่อนไขเดียว: **ต้องอยู่ในระยะ ~10 studs** (ทดสอบที่ 58m → ❌ ไม่เข้า)
  และก้อนต้องมี ProximityPrompt (ก้อน "ไม่มี prompt" → ❌ เสมอ)
...
- **⚠ เกมมีจังหวะ "โหลด" ที่ทำอะไรไม่ได้เลย** — ตอนเข้าเกม/วาร์ปโซน/ตายเกิดใหม่ จะมีแถบ
  `2,437 / 4,937` (โหลดคริสตัลเข้าแมพ) ช่วงนั้น **Character/Backpack ยังไม่มา**
  อาการใน log: `ห่าง -1m`, `ถึงแล้วใน 0.00 วิ (ห่างเป้า -1.0)`, `⚠ ไม่เจอขวานในกระเป๋า`
  → ต้องรอ HumanoidRootPart + Humanoid.Health>0 + Backpack + Inventory + ก้อนในแมพ ≥20
  ก่อนเริ่มฟาร์ม (AutoFarm v4.0) — ไม่ใช่อาการโดน throttle! (ขายยังทำงานปกติ)
- **server มีลิมิตคำสั่งต่อวินาที** — v3.4 ยิง remote ใส่ของตก 120 คำสั่งรวด (ทุกก้อน x3 รอบ
  ห่าง 0.08 วิ) → server เมินทุกคำสั่งรวมถึงการเก็บปกติ ต้องเว้นจังหวะ ≥0.3 วิ เสมอ
- **⚠ สคริปต์ 2 ตัวแย่งเขียน CFrame กัน** — ItemESP เปิด "บิน" พร้อม AutoFarm ทำงาน = วาปไม่ถึงเป้า
  (log ขึ้น `ถึงแล้วใน 3.07 วิ (ห่างเป้า 28.6)` = อาการนี้) → ใช้ธง `_G.AF75_PIN` ให้ ESP หลบ
  (AutoFarm v2.4 / ESP v2.18) — บทเรียน: ระบบตรึงตำแหน่งต้องมีเจ้าเดียวเสมอ
- **Roblox โชว์ ProximityPrompt ให้ "ก้อนใกล้สุดก้อนเดียว"** — ในโซนก้อนแน่น ปุ่มของก้อนเป้าหมาย
  ไม่มีวันขึ้น (log เห็นสลับ Starsapphire↔Cerulime ทั้งที่ระยะไม่เปลี่ยน) → **ห้ามพึ่ง `PromptShown`
  อย่างเดียว** ให้คำนวณระยะเทียบ `MaxActivationDistance` เอง (AutoFarm v2.6)
- **`MaxActivationDistance` ต่างกันตามขนาดก้อน**: เล็ก 9-11 / กลาง 11-12 / ใหญ่ 14-15
  **`HoldDuration` ตามเทียร์**: T1=1.0 T3=2.0 T4=2.8 T5=4.0 T6=5.0
- **`Max=0` = "เรายังไม่เข้าใกล้ก้อนนั้น" ไม่ใช่ก้อนเสีย!** (LoadSpy 2026-08-06 พิสูจน์:
  ทั้งแมพ 986 ก้อน มี Max>0 แค่ **1 ก้อน** คือก้อนที่เรายืนอยู่ข้างๆ)
  เกม "ติดอาวุธ" prompt (ตั้ง Max 9-15) เฉพาะก้อนใกล้ตัวเท่านั้น → เลือกเป้าจากระยะไกล
  จะเห็น Max=0 เป็นเรื่องปกติ **ต้องวาปไปถึงก่อนแล้วรอ ~0.5-3 วิให้ prompt ติด** (AutoFarm v4.1)
  ⚠ v2.6-v4.0 ตีความผิดว่า Max=0 = ฝังดิน เลยส่งก้อนดีๆ ไปฟันขวานเปล่าเป็นสิบวินาที
- `LP.loaded` (BoolValue) = false ตลอด ใช้เป็นสัญญาณ "โหลดเสร็จ" ไม่ได้
- **ก้อนมี 2 แบบ**: "หยิบ" (กด prompt เข้าเลย) กับ **"ต้องขุดด้วยขวาน"** (ก้อนเล็กแต่แพง T6)
  ก้อนขุด: ระยะสั้นกว่าระยะหยิบ (เกมขึ้น "ไกลเกินไป! เข้าใกล้เพื่อขุด ⛏ [x4]") ต้องยืน ~3 studs
  + **ขุดแตกแล้วของร่วงเป็นก้อนตกพื้น** (`DroppedCrystals`, attr `DroppedByUserId`) ต้องตามเก็บอีกที
  ไม่งั้นก้อนหายแต่น้ำหนักไม่ขึ้น (AutoFarm v2.2 แก้: ยืนชิด + ตามเก็บของตกรัศมี 120)
