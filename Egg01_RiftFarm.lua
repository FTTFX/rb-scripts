-- Egg01 Rift Farm v1.4 -- ยืนยัน Prompt ด้วยชื่อ/UID หรือระยะ UID แบบปลอดภัย
...
local function choosePrompt(t)
 ...
 local candidates={}
 for _,v in ipairs(prompts()) do
  local d=(v.pos-t.pos).Magnitude; local near=(v.pos-me.Position).Magnitude; local ok,why=promptProof(v.p,t)
  if d<=8 and near<=14 then candidates[#candidates+1]={p=v.p,d=d,proof=ok and why or nil} end
 end
 table.sort(candidates,function(a,b)return a.d<b.d end)
 for _,c in ipairs(candidates) do if c.proof then return c.p,c.d,c.proof end end
 -- หาก Prompt ไม่เปิดชื่อ/UID: ยอมรับเฉพาะจุดที่ใกล้ UID มากมาก และห่างจาก Prompt รองลงมาชัดเจน
 local a,b=candidates[1],candidates[2]
 if a and a.d<=4 and (not b or b.d-a.d>=3) then return a.p,a.d,"position" end
 local detail=a and string.format("closest=%.1f gap=%s",a.d,b and string.format("%.1f",b.d-a.d) or "-") or "ไม่มี Prompt ใน 8"
...
local function one(t)
 ...
 local pick,md,proof,detail=choosePrompt(t)
 ...
 if not pick then
  for _,off in ipairs({Vector3.new(4,0,0),Vector3.new(-4,0,0),Vector3.new(0,0,4),Vector3.new(0,0,-4)}) do
   if walk(t.pos+off,2,2) then stop(); pick,md,proof,detail=choosePrompt(t); if pick then break end end
  end
 end
...
local title=Instance.new("TextLabel",f); title.Size=UDim2.new(1,-78,0,28); title.Position=UDim2.new(0,10,0,4); title.BackgroundTransparency=1; title.Text="Egg01 Rift Farm v1.4 — PROMPT PROBE"; title.TextColor3=Color3.fromRGB(220,170,255); title.Font=Enum.Font.GothamBold; title.TextSize=13; title.TextXAlignment=Enum.TextXAlignment.Left
