...
local function target()
 ...
 local best; for id,row in pairs(rec) do local p=typeof(row)=="table" and pos(row); if p and rift(row) and row.State~="Carried" then local d=(p-root.Position).Magnitude; if not best or d<best.d then best={uid=row.Uid or id,cat=row.AssetCategory or row.AssetName or "Rift",pos=p,d=d} end end end
 if best then say(string.format("RIFT TARGET %s d=%.0f",best.cat,best.d)) else say("ยังไม่มี Rift egg ใน FieldEggSnapshot") end; return best
end
...
local function one(t)
 ...
 if S.home then walk(S.home,60,120); stop(); say("กลับ HOME") else say("เก็บแล้ว — ไม่มี HOME จึงหยุด") end
end
...
say("HOME (ถ้าต้องกลับฐาน) → START | เก็บเฉพาะ record ที่มี Rift")
