-- Egg01 Motion Lab v1.1 -- วัดเดินปกติ / แรงผลักสั้น / แรงผลัก + Hop เบรก
...
local function run(name,mode)
    ...
    while S.run and os.clock()-began<45 do
        ...
        if d>prevD+12 then rollback=rollback+(d-prevD) end
        if now-sampleT>=.6 then
            local actual=(r.Position-samplePos).Magnitude/math.max(now-sampleT,.01)
            say(string.format("t=%.1f d=%.0f actual=%.0f phys=%.0f push=%s",now-began,d,actual,vel,pushed and "Y" or "N"))
            samplePos,sampleT=r.Position,now
        end
        prevD=d
...
local title=Instance.new("TextLabel",f);title.Size=UDim2.new(1,-50,0,30);title.Position=UDim2.new(0,10,0,2);title.BackgroundTransparency=1;title.Text="Egg01 Motion Lab v1.1 — Walk / Push / Hop";title.TextColor3=Color3.fromRGB(180,220,255);title.Font=Enum.Font.GothamBold;title.TextSize=13;title.TextXAlignment=Enum.TextXAlignment.Left
...
normal.MouseButton1Click:Connect(function()startTest("WALK","WALK")end);push.MouseButton1Click:Connect(function()startTest("PUSH","PUSH")end);hop.MouseButton1Click:Connect(function()startTest("PUSH+HOP","HOP")end);halt.MouseButton1Click:Connect(function()S.run=false;stop();say("STOP")end);copy.MouseButton1Click:Connect(function()local c=setclipboard or toclipboard;if c then pcall(c,"=== Egg01 Motion Lab v1.1 ===\n"..table.concat(S.lines,"\n"));copy.Text="OK";task.delay(1,function()if copy.Parent then copy.Text="COPY"end end)end end)
