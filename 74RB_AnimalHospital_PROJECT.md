...
- **v6.59→v6.63 กรอบโรงพยาบาล (`HOSP`) = กฎของก๊อปกลาง**: เกมจอด copy ของ**ทุกชนิด** (ยา/กาแฟ/ปืน/ห้อง/เครื่อง/prompt) ไว้นอกอาคาร (จุดกำเนิด (0,~,0) เป็นหลัก — MoveSpy จับได้ prompt ก๊อปบางตัวกดแล้วเกม**ดูดตัวเราไปที่เครื่องก๊อป**) → กฎสุดท้าย: **ของจริงต้องอยู่ในกรอบ X -175..-85, Z -145..130** (`inCopyZone`/`promptFake`) ผูกที่ pressPrompt + blind-fire + tpTo + ตัวเลือกเป้าทุก loop + ตาข่ายดึงตัวกลับ (`SAFE_POS`) ; ชั้นเสริมยา: `MED_SPOTS` พิกัดจริง ±12
