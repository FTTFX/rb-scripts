...
- **v6.24→v6.29: โหมดรับผี — ยิงปืนรอเช็คอินเสร็จก่อน** (ผู้ใช้ขอ: เอาแต้มเช็คอินจากผีด้วย): `ghostWaitCheckin(m)` — **กฎสุดท้าย (v6.29, ผู้ใช้เลือก): ยิงเฉพาะผีที่ `CompletedCheckIn=<ชื่อผู้เล่นใครก็ได้>` (string ไม่ว่าง) เท่านั้น ไม่มีข้อยกเว้น** ; GhostCiSpy พิสูจน์: ผีเช็คอินเสร็จได้ `CompletedCheckIn` เหมือนคนไข้เป๊ะ ; ประวัติ: v6.24 จำกัดโซน 60 studs (ผีไกลโดนยิงก่อนถึง) → v6.27 รอทุกระยะแต่ยกเว้น SkippedCheckIn/InBed → v6.28 เพิ่ม `validShootPos` (Y>-50, ไม่สูงกว่าเรา 30, <200 — ยังใช้อยู่) — สองรุ่นนี้ยังหลุดบินไปยิงผี event ที่เกมจอดนอกแมพ เพราะ attr ยกเว้นครบ ; **v6.31: ปืนคุมด้วยปุ่ม "ยิงผี" (GUNKILL_ON) ของมันเอง** — "รับผี" แค่เพิ่มเงื่อนไขรอเช็คอิน: GHOSTCI_ON=false → ยิงปกติไม่รอ, GHOSTCI_ON=true → รอ CompletedCheckIn (v6.30 เคยผูกปืนกับรับผี = เข้าใจผิด ย้อนแล้ว)
...
- **v6.59→v6.63 กรอบโรงพยาบาล (`HOSP`) = กฎของก๊อปกลาง**: เกมจอด copy ของ**ทุกชนิด** (ยา/กาแฟ/ปืน/ห้อง/เครื่อง/prompt) ไว้นอกอาคาร (จุดกำเนิด (0,~,0) เป็นหลัก — MoveSpy จับได้ prompt ก๊อปบางตัวกดแล้วเกม**ดูดตัวเราไปที่เครื่องก๊อป**) → กฎสุดท้าย: **ของจริงต้องอยู่ในกรอบ X -230..-85, Z -160..150** (v6.68 — คนไข้จริง R6 อยู่ X=-182 เคยหลุดกรอบแคบ) (`inCopyZone`/`promptFake`) ผูกที่ pressPrompt + blind-fire + tpTo + ตัวเลือกเป้าทุก loop + ตาข่ายดึงตัวกลับ (`SAFE_POS`) ; ชั้นเสริมยา: `MED_SPOTS` พิกัดจริง ±12
