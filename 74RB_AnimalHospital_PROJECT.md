...
- **v6.24→v6.29: โหมดรับผี — ยิงปืนรอเช็คอินเสร็จก่อน** (ผู้ใช้ขอ: เอาแต้มเช็คอินจากผีด้วย): `ghostWaitCheckin(m)` — **กฎสุดท้าย (v6.29, ผู้ใช้เลือก): ยิงเฉพาะผีที่ `CompletedCheckIn=<ชื่อผู้เล่นใครก็ได้>` (string ไม่ว่าง) เท่านั้น ไม่มีข้อยกเว้น** ; GhostCiSpy พิสูจน์: ผีเช็คอินเสร็จได้ `CompletedCheckIn` เหมือนคนไข้เป๊ะ ; ประวัติ: v6.24 จำกัดโซน 60 studs (ผีไกลโดนยิงก่อนถึง) → v6.27 รอทุกระยะแต่ยกเว้น SkippedCheckIn/InBed → v6.28 เพิ่ม `validShootPos` (Y>-50, ไม่สูงกว่าเรา 30, <200 — ยังใช้อยู่) — สองรุ่นนี้ยังหลุดบินไปยิงผี event ที่เกมจอดนอกแมพ เพราะ attr ยกเว้นครบ ; **v6.30: GHOSTCI_ON=false = ปืนไม่ยิงอะไรเลย** (ผู้ใช้ขอ — ปืนทำงานเฉพาะโหมดรับผี แม้ปุ่มยิงผีเปิดอยู่)
