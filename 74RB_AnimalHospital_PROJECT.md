...
| **คนเป็นลม (faint)** | NPC ล้ม มี PP `'Carry'` (obj=spine.002 บนตัว) ; ตอนถูกอุ้มมี attr `CarriedBy=<userid>` ; คนไข้และคนเยี่ยมเป็นได้ทั้งคู่ | v4.70: auto อุ้มส่งห้อง — บินไปหา → Carry → บินไป DesignatedRoom → กด prompt วาง (บนตัว NPC ก่อน, ไม่มีก็หา Place/Drop/Lay/Put ในห้อง — **ชื่อ prompt วางยังไม่ยืนยัน** ดูจากสถานะหัว GUI "วาง (X)") |
