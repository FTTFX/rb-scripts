...
- logic: มี `Skinwalker` ใกล้ `Workspace.Misc.CheckIn` (ระยะ <20 studs) **และ** ActionText=='Close' → `fp(pp)` ปิด
- self-limiting: ปิดแล้ว ActionText='Open' → ไม่ยิงซ้ำ/ไม่เผลอเปิด ; ปรับ `COUNTER_RANGE` ถ้าจับไกล/ใกล้ไป
