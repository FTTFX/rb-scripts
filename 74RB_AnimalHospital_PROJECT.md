...
- **v6.59 โซนจอดของก๊อปกลางแมพ = จุดกำเนิด (0,~,0)** — เกมจอด copy ของ (ยา/แท่นถัง/prompt) ไว้แถวนี้ (MoveSpy จับได้: บอทบินไป (0,6,0) รัวๆ ตอนรักษา) ; โรงพยาบาลจริงอยู่ X -157..-97 ทั้งหมด → `tpTo` และ `findPickup` แบนเป้าในรัศมี 40 จากจุดกำเนิด
