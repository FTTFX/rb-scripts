-- 74RB_AnimalHospital.lua — ESP + AUTO รักษา + ชัตเตอร์ + ดับไฟ + NPC เร็ว  (v6.25 ดิ้นหลุดผีเตียง: กด E ค้าง 0.08s/ครั้ง — เดิมกดปล่อยใน 1 เฟรมสั้นไปเกมนับไม่ทัน ดิ้นไม่หลุดตาย)
...
--        เคาน์เตอร์ 2 ช่องอยู่คนละผนัง (มุมฉาก) — ใช้กรอบสี่เหลี่ยมห้องหลบ (รอ MARK 4 มุมจากผู้ใช้)
local HIDE_BOX = nil   -- ใส่เป็น {x1=..., x2=..., z1=..., z2=...} (min/max) เมื่อได้พิกัดจาก CheckinSpy MARK
local function inCheckinHall(p)
    if not HIDE_BOX then return true end   -- ยังไม่รู้กรอบห้อง = นับทุกตัวแบบเดิม (ห้ามตัดมั่ว)
    return not (p.X > HIDE_BOX.x1 and p.X < HIDE_BOX.x2 and p.Z > HIDE_BOX.z1 and p.Z < HIDE_BOX.z2)
end
...
local function checkinPending()
    ...
    -- v5.71: สลับช่องทุก 0.7 วิ (ผู้ใช้: 2 วิช้าไป — server เช็คแค่ว่ายืนอยู่ตรงนั้นตอนส่งเอกสาร
    --        วาปไปมาถี่ๆ = ได้เอกสารทั้ง 2 ช่องเกือบพร้อมกัน)
    return list[math.floor(os.clock() / 0.7) % #list + 1].pos
end
...
-- v6.25: เดิมกด-ปล่อยใน 1 เฟรม (~0.016s) สั้นเกิน เกมนับไม่ทัน = ดิ้นไม่หลุดตาย (ผู้ใช้เจอ)
--        → กดค้าง 0.08s ให้เกมเห็นแน่ๆ ค่อยปล่อย แล้วเว้น 0.04s ก่อนกดใหม่
...
local function tpTo(pos, speedOpt, noGap)   -- v5.05: speedOpt override ความเร็ว | v5.32: noGap=ยอมประชิดผี (จ่ายยาผิด)
    ...
    if TP_ON then
        ...
        local SLIDE_SPEED = speedOpt or 200  -- studs/s แนวราบ (v6.00 ผู้ใช้ลด 1000→200)
...
local NO_DISCARD = { "taser", "extinguisher", "gun", "cola", "coffee", "syrup", "scanner", "choco" }   -- v6.07: +ช็อกโกแลต (item ใหม่ ห้ามทิ้ง — match "choco" ครอบ Chocolate/Choco Bar)
...
title.Text, title.Font, title.TextSize = "AH74 v6.25", Enum.Font.GothamBold, 14   -- โชว์เวอร์ชัน+สถานะบนหัว GUI
...
setStatus = function(s)
    ...
    if not deadLock then
        ...
        title.Text = "v6.25 " .. sb .. " " .. lastStatus
    end
end
...
-- ===== v5.83 ปุ่มแยก "Ghost": ฆ่า Ghost (Anomaly+Ghost) ด้วย Scanner (ผู้ใช้สั่งแยก — ตัวนี้ไม่ปกติ) =====
-- ผู้ใช้พิสูจน์: remote เดียวกับปืน PlayShootEffect ; เงื่อนไข: ยิงจาก "ด้านบน" + ห้ามมีคนไข้/คนเยี่ยมข้างๆ
task.spawn(function()
    ...
    while _G.AH74_GEN == MYGEN do
        if GHOSTKILL_ON and not WORKING and not busyBefore(5) then
            ...
            if re and npcs and me and scan then
                ...
                if g then
                    local clear = true   -- คนไข้/คนเยี่ยมใกล้ผี <8 = ห้ามยิง (ลูกหลง)
                    for _, m in ipairs(npcs:GetChildren()) do
                    ...
                    if clear then
                        ...
                        elseif r and ghead.Parent then
                            ...
                            task.wait(1.5)
                            local h3 = g:FindFirstChildOfClass("Humanoid")
                            if h3 and h3.Health > 0 and g.Parent and g:GetAttribute("Anomaly") then
                                shotAt[g] = os.clock()
                                setStatus("Ghost ไม่ตาย — รอลองใหม่")
                            end
                        end
                    else
                    ...
                    if hider and hider.Parent and hider:GetAttribute("Anomaly") then
                        ...
                        if ext then
                            ...
                            local function mouse(down)
                                if mDown == down then return end
                                mDown = down
                                pcall(function() VIM:SendMouseButtonEvent(vp.X/2, vp.Y/2, 0, down, game, 1) end)
                            end
                            local oldCamType = cam.CameraType
                            local t0 = os.clock()
                            local r = hrp()
...
-- ===== v5.75 auto ซ่อมกล้องวงจรปิด: prompt 'Fix Camera' ใต้ Misc.Cameras*/<โซน>/MainLook (CameraSpy ยืนยัน) =====
        ...
        if CAMFIX_ON and fp and not WORKING and not busyBefore(7) then
            ...
            if misc then
                for _, fname in ipairs({ "Cameras", "Cameras2" }) do
                    ...
                    if fo then
                        for _, d in ipairs(fo:GetDescendants()) do
                            if d:IsA("ProximityPrompt") and d.Enabled and d.ActionText == "Fix Camera" then
                                pps[#pps+1] = d
                            end
                        end
                    end
            ...
            if #pps > 0 then
                ...
                for _, pp2 in ipairs(pps) do
                    ...
                    if pos and hrp() and pp2.Enabled then
                        ...
                        local t0 = os.clock()
                        while hrp() and os.clock() - t0 < 0.4 do   -- ลอยค้างใต้ prompt (-2 กันหัวมุดเพดาน)
                            local r2 = hrp()
                            r2.CFrame = CFrame.new(pos + Vector3.new(0, -2, 0))
                            r2.AssemblyLinearVelocity = Vector3.zero
                            task.wait()
                        end
                        pressPrompt(pp2, function() return not pp2.Parent or not pp2.Enabled end)
                        setStatus((pp2.Parent and pp2.Enabled) and "ซ่อมกล้องไม่ติด" or "ซ่อมกล้องแล้ว ✓")
                        local r3 = hrp()
                        if r3 then r3.CFrame = back; r3.AssemblyLinearVelocity = Vector3.zero end
                    end
                end
                WORKING = false
            end
