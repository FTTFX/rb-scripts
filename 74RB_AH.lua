...
local function checkinPending()
    ...
    local function nearCounter(p)
        ...
        for _, c in ipairs(counters) do
            ...
            if d < 10 and (not best or d < bd) then best, bd = c, d end   -- v6.42: เหลือ 10 (ผู้ใช้ขอ — 30 กว้างไป)
        end
        return best
    end
...
local function bedApplyPP(room)
    ...
    local first, other
    for _, p in ipairs(room:GetDescendants()) do
        if p:IsA("ProximityPrompt") and p.ActionText == "Apply Treatment"
           and not inCopyZone(partPos(p.Parent)) then   -- v6.67: Main ลอยนอกแมพ = ตัดทิ้งตั้งแต่ต้น
            if p.Enabled then
