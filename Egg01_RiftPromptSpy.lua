-- Egg01 Rift Prompt Spy v1.1 -- ดู instance/trigger/snapshot เท่านั้น ไม่ยิง Steal
...
local title=Instance.new("TextLabel",f);title.Size=UDim2.new(1,-50,0,30);title.Position=UDim2.new(0,10,0,3);title.BackgroundTransparency=1;title.Text="Egg01 Rift Prompt Spy v1.1 — WATCH TRIGGER";title.TextColor3=Color3.fromRGB(225,180,255);title.Font=Enum.Font.GothamBold;title.TextSize=14;title.TextXAlignment=Enum.TextXAlignment.Left
...
local near=b("NEAR",10,74,Color3.fromRGB(50,100,180));local dumpB=b("DUMP",90,74,Color3.fromRGB(35,145,75));local clear=b("CLEAR",170,74,Color3.fromRGB(75,75,80));local copy=b("COPY",250,74,Color3.fromRGB(75,75,80));local watchB=b("WATCH",330,74,Color3.fromRGB(130,95,45));local close=b("X",630,34,Color3.fromRGB(145,50,65))
...
near.MouseButton1Click:Connect(list);dumpB.MouseButton1Click:Connect(dump);watchB.MouseButton1Click:Connect(watch);clear.MouseButton1Click:Connect(function()S.lines={};box.Text=""end);copy.MouseButton1Click:Connect(function()local c=setclipboard or toclipboard;if c then pcall(c,"=== Egg01 Rift Prompt Spy v1.1 ===\n"..table.concat(S.lines,"\n"));copy.Text="OK";task.delay(1,function()if copy.Parent then copy.Text="COPY"end end)end end);close.MouseButton1Click:Connect(function()for _,c in ipairs(S.conns)do pcall(function()c:Disconnect()end)end;gui:Destroy();_G.EGG01_RIFT_PROMPT_SPY=nil end)
say("ยืนติด Pterodactyl → DUMP | หรือ WATCH → กด E เอง 1 ครั้ง → COPY")
