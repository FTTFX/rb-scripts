-- Egg01 Experiment Farm v2.12 (restore) — RiftMachine→วาฬ + ตี Drone แบบเดิมที่เคยใช้ได้
-- คืนจากช่วงที่ตี DroneVisual ได้จริง / ตัดตัวที่สแกนแคบแล้วไม่เจอโดรน
...
-- ขั้น1 RiftMachine → ขั้น2 วาฬ (แบบ 2.12)
...
local function leaveTreadmill()
    ...
    if h and r then
        ...
        walk(r.Position+dir*16,3,6,nil)
    end
    stop()
end
...
local function farm5min()
    ...
    while S.run and os.clock()<deadline do
        ...
        local all=robots()
        -- เลือกเฉพาะในระยะตีได้ (ใกล้ผู้เล่น)
        local near={}
        ...
        if #near==0 then
            ...
            say(string.format("พบ %d ตัว — ตีใกล้สุด d=%.0f",#near,near[1].d))
...
local function beginAuto()
    ...
    say("AUTO ON — ขั้นอีเวนต์: RiftMachine → วาฬ | นอก=เครื่องวิ่งรอ")
    task.spawn(function()
...
say("เปิดสคริปต์ = AUTO | ขั้น1 RiftMachine → ขั้น2 วาฬ | PATH=ทดสอบ")
task.spawn(boot)
